...
const weeklyChartData = computed(() => {
    const days = props.weeklySales || [];
    const salesByDate = {};
    for (const d of days) {
        salesByDate[d.date] = parseFloat(d.total);
    }
    const data = [];
    for (let i = 6; i >= 0; i--) {
        const date = new Date();
...
const topProductsChartData = computed(() => {
    const items = props.topProducts || [];
    const colors = ['#F37021', '#FFC20E', '#E31E24', '#16a34a', '#6366f1'];
    return {
        labels: items.map(p => p.product?.name || 'Producto'),
        datasets: [{
            data: items.map(p => parseInt(p.total_qty)),
...
<template>
    ...
    <AuthenticatedLayout>
        <div class="pb-8">
            <!-- Header Mejorado -->
            <div class="-mx-3 sm:-mx-6 -mt-3 sm:-mt-6 bg-gradient-to-br from-primary via-orange-500 to-orange-700 text-white p-8 sm:p-12 shadow-[0_20px_50px_rgba(243,112,33,0.3)] relative overflow-hidden mb-10">
                ...
                <div class="w-full flex flex-col md:flex-row justify-between items-center px-4 relative z-10 gap-8">
                    <div class="animate-fade-in-up">
                        <h1 class="text-5xl font-black italic uppercase tracking-tighter font-poppins leading-none">
                            ¡Hola, {{ $page.props.auth.user.name }}! 👋
                        </h1>
                        <p class="text-orange-100 mt-4 font-bold text-xl capitalize flex items-center gap-3 bg-white/10 backdrop-blur-md w-fit px-6 py-2 rounded-2xl border border-white/20">
                            <span class="text-2xl">📅</span> {{ formatDate(currentTime) }}
                        </p>
                    </div>
                    <div class="text-center md:text-right animate-fade-in-up" style="animation-delay: 0.1s">
                        <div class="text-7xl font-black font-mono tracking-tighter drop-shadow-2xl">
                            {{ formatTime(currentTime) }}
                        </div>
                        <div v-if="activeRegister" class="inline-flex items-center gap-3 bg-white/20 backdrop-blur-xl px-6 py-2.5 rounded-[1.5rem] text-base mt-6 font-black border border-white/30 shadow-2xl">
                        ...
                        <div v-else class="inline-flex items-center gap-3 bg-red-500/20 backdrop-blur-xl px-6 py-2.5 rounded-[1.5rem] text-base mt-6 font-black border border-red-500/30 text-red-100 animate-pulse shadow-2xl">
                            <span class="text-xl">🔒</span> CAJA CERRADA
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-12 mt-8">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    <Card>
                        <h2 class="text-lg font-black text-text uppercase font-poppins mb-4">Ventas Semanales</h2>
                        <div class="h-64">
                            <Bar v-if="weeklySales?.length" :data="weeklyChartData" :options="chartOptions" />
                            <EmptyState v-else icon="📊" title="Sin datos" message="No hay ventas esta semana." />
                        </div>
                    </Card>
                    <Card>
                        <h2 class="text-lg font-black text-text uppercase font-poppins mb-4">Top Productos</h2>
                        <div class="h-64">
                            <Doughnut v-if="topProducts?.length" :data="topProductsChartData" :options="doughnutOptions" />
                            <EmptyState v-else icon="🏆" title="Sin datos" message="No hay productos vendidos esta semana." />
                        </div>
                    </Card>
                </div>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6">
                        <Link :href="route('pos.index')" class="group relative overflow-hidden bg-gradient-to-br from-primary to-orange-600 rounded-[2.5rem] p-8 text-white shadow-xl hover:shadow-2xl transition-all hover:-translate-y-2 flex flex-col items-center text-center">
                        ...
                        <Link :href="route('kitchen.index')" class="group relative overflow-hidden bg-gradient-to-br from-secondary to-yellow-500 rounded-[2.5rem] p-8 text-white shadow-xl hover:shadow-2xl transition-all hover:-translate-y-2 flex flex-col items-center text-center">
                            <div class="absolute top-0 right-0 w-32 h-32 bg-white/20 rounded-full -mr-12 -mt-12 group-hover:scale-150 transition-transform duration-300"></div>
                        ...
                        <Link v-if="isAdmin" :href="route('cash-register.index')" class="group relative overflow-hidden bg-gradient-to-br from-green-600 to-emerald-700 rounded-[2.5rem] p-8 text-white shadow-xl hover:shadow-2xl transition-all hover:-translate-y-2 flex flex-col items-center text-center">
                            <div class="absolute top-0 right-0 w-32 h-32 bg-white/20 rounded-full -mr-12 -mt-12 group-hover:scale-150 transition-transform duration-300"></div>
                            <span class="text-6xl mb-6 block relative z-10 group-hover:scale-110 transition-transform">💰</span>
                        ...
                        <Link v-if="isAdmin" :href="route('reports.index')" class="group relative overflow-hidden bg-gradient-to-br from-blue-600 to-indigo-700 rounded-[2.5rem] p-8 text-white shadow-xl hover:shadow-2xl transition-all hover:-translate-y-2 flex flex-col items-center text-center">
                            <div class="absolute top-0 right-0 w-32 h-32 bg-white/20 rounded-full -mr-12 -mt-12 group-hover:scale-150 transition-transform duration-300"></div>
                        ...
                        <Link :href="route('waiter.index')" class="group relative overflow-hidden bg-gradient-to-br from-purple-600 to-fuchsia-700 rounded-[2.5rem] p-8 text-white shadow-xl hover:shadow-2xl transition-all hover:-translate-y-2 flex flex-col items-center text-center">
                            <div class="absolute top-0 right-0 w-32 h-32 bg-white/20 rounded-full -mr-12 -mt-12 group-hover:scale-150 transition-transform duration-300"></div>
                        ...
                        <Link :href="route('products.index')" class="group relative overflow-hidden bg-gradient-to-br from-orange-400 to-yellow-600 rounded-[2.5rem] p-8 text-white shadow-xl hover:shadow-2xl transition-all hover:-translate-y-2 flex flex-col items-center text-center">
                            <div class="absolute top-0 right-0 w-32 h-32 bg-white/20 rounded-full -mr-12 -mt-12 group-hover:scale-150 transition-transform duration-300"></div>
                            <span class="text-6xl mb-6 block relative z-10 group-hover:scale-110 transition-transform">🍗</span>
