  ...
  <AuthenticatedLayout title="Nuevo Producto">
    <template #header>
      <div class="flex items-center gap-4">
        ...
        <div>
          <h2 class="text-2xl font-black text-text tracking-tight uppercase">Nuevo Producto</h2>
          <p class="text-xs text-gray-400 font-bold">Agrega un producto al catálogo</p>
        </div>
      </div>
    </template>
                ...
                <div class="space-y-3">
                  <div v-for="(variant, index) in form.variants" :key="index"
                    ...
                    <label class="flex items-center gap-1 cursor-pointer" title="Imagen de variante">
                      <input type="file" accept="image/*" @change="e => handleVariantImage(e, index)" class="hidden" :id="'variant-img-' + index" />
                      <span @click="document.getElementById('variant-img-' + index)?.click()"
