...
const form = useForm({
  ...
  variants: props.product.variants?.length
    ? props.product.variants.map(v => ({ id: v.id, name: v.name, price: String(v.price), is_active: v.is_active ?? true }))
    : []
})
...
const handleVariantImage = (e, index) => {
  const file = e.target.files[0]
  if (!file) return
  variantImages.value[index] = file
}

const addVariant = () => {
  form.variants.push({ name: '', price: '', is_active: true })
}
...
const submit = () => {
  if (Object.keys(variantImages.value).length > 0) {
    ...
    Object.entries(variantImages.value).forEach(([i, file]) => {
      fd.append(`variant_images[${i}]`, file)
    })

    router.post(route('products.update', props.product.id), fd, {
      preserveScroll: true,
      onSuccess: () => {
          ...
          <div class="space-y-6">
            <div class="bg-white rounded-3xl shadow-lg overflow-hidden">
              ...
              <div class="p-4">
                ...
                <p v-if="form.errors.image" class="text-danger text-xs mt-2 font-bold">{{ form.errors.image }}</p>
              ...
              <div class="p-6">
                ...
                <div class="space-y-3">
                  <div v-for="(variant, index) in form.variants" :key="index"
                    class="flex gap-3 items-center p-4 bg-gray-50 rounded-xl group hover:bg-gray-100 transition-colors flex-wrap">
                    ...
                    <input v-model="variant.name" type="text" placeholder="Nombre (ej: 1/4, 1/2, Entero)"
                      class="flex-1 min-w-[100px] px-3 py-2 bg-white border border-gray-200 rounded-lg font-medium text-sm focus:ring-2 focus:ring-orange-100 focus:border-primary transition-all" />
                    <div class="relative">
                    ...
                    <label class="flex items-center gap-1 cursor-pointer" title="Imagen de variante">
                      <input type="file" accept="image/*" @change="e => handleVariantImage(e, index)" class="hidden" :id="'edit-variant-img-' + index" />
                      <span @click="document.getElementById('edit-variant-img-' + index)?.click()"
                        class="text-[10px] font-bold text-gray-400 uppercase hover:text-primary transition-colors cursor-pointer">
                        {{ variantImages[index] ? '📷' : '📁' }}
                      </span>
                    </label>
                    <label class="flex items-center gap-1.5 cursor-pointer" title="Variante activa">
                      <input type="checkbox" v-model="variant.is_active" class="rounded border-gray-300 text-primary focus:ring-primary/30" />
                      <span class="text-[10px] font-bold text-gray-400 uppercase">Activo</span>
                    </label>
