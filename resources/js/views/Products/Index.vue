    ...
    <div class="pb-6">
      <div class="space-y-8">
        ...
        <Card no-padding>
          ...
          <!-- Vista de Tarjetas -->
          <div v-if="viewMode === 'cards'" class="p-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
            <div v-for="product in filteredProducts" :key="product.id" 
              class="group bg-white rounded-[2.5rem] border-2 border-white shadow-xl shadow-orange-500/5 hover:border-primary/20 hover:shadow-2xl hover:shadow-orange-500/10 hover:-translate-y-2 transition-all duration-300 overflow-hidden flex flex-col relative">
              
              <div class="aspect-square relative overflow-hidden bg-gray-50/50 p-8 flex items-center justify-center">
                <img v-if="product.image_url" :src="product.image_thumbnail_url || product.image_url" :alt="product.name" 
                ...
                <span v-else class="text-8xl group-hover:scale-110 transition-transform duration-500 grayscale opacity-20">🍗</span>
                
                <div class="absolute top-6 right-6">
                  <span :class="product.is_active ? 'bg-green-500 shadow-green-200' : 'bg-gray-400 shadow-gray-200'" 
                    class="px-4 py-1.5 rounded-full text-[10px] font-black text-white uppercase tracking-widest shadow-lg">
              ...
              <div class="p-8 flex-1 flex flex-col">
                <p class="text-[10px] font-black text-primary uppercase tracking-[0.2em] mb-2">{{ product.category?.name || 'Sin Categoría' }}</p>
                <h3 class="text-xl font-black text-text uppercase leading-tight font-poppins min-h-[3.5rem] line-clamp-2">{{ product.name }}</h3>
                
                <div class="mt-6 flex items-center justify-between bg-gray-50 p-4 rounded-2xl border border-gray-100">
                  <div class="flex flex-col">
                    <span class="text-[9px] font-black text-gray-400 uppercase tracking-widest">Precio Base</span>
                    <span class="text-2xl font-black text-text font-mono">Bs. {{ getProductPrice(product) }}</span>
                  </div>
                  <div v-if="hasVariants(product)" class="bg-primary/10 px-3 py-1.5 rounded-xl text-primary text-[10px] font-black uppercase tracking-widest border border-primary/20">
                    {{ product.variants.length }} Variantes
                  </div>
                </div>

                <div class="mt-8 pt-6 border-t border-gray-100 flex gap-3">
                  <PrimaryButton :href="route('products.edit', product.id)" class="flex-1 !py-3 !text-[11px] !rounded-2xl">
                    ✏️ Editar
                  </PrimaryButton>
                  <DangerButton @click="showDeleteId = product.id" class="!py-3 !px-5 !rounded-2xl">
                    🗑️
                  </DangerButton>
                </div>
              </div>
                  ...
                  <td class="px-6 py-4">
                    <div class="flex items-center gap-4">
                      <div class="w-12 h-12 rounded-xl bg-gray-50 flex items-center justify-center overflow-hidden border border-gray-100">
                        <img v-if="product.image_url" :src="product.image_thumbnail_url || product.image_url" class="w-full h-full object-contain" loading="lazy">
